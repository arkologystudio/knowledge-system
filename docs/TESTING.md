...
- `test/rls-per-artifact-labels.test.ts` — v129 per-artifact labels, real Postgres only (skips without `DATABASE_URL`; PGLite has no policies and its `withRlsScope` is a pass-through, so there is nothing to prove there). Pins the claim source-level isolation cannot express: TWO ARTIFACTS IN ONE SOURCE, adjacent and linked, resolve differently for the same caller because they carry different labels. Also: the trigger labels every new page with its source (without it a synced page is invisible to everyone), chunks inherit the parent's labels so no passage leaks from an unlabelled page, an edge into an unreadable artifact is dropped silently even though its near endpoint is visible, label metadata is itself scoped so a guest cannot enumerate the org's other audiences, and a zero-grant or unset clearance returns nothing.
- `test/classify-page-op.test.ts` — `classify_page` / `get_page_spaces`. Two layers: authority + validation against a stub engine (no DB), then a real round-trip when `GBRAIN_DATABASE_URL` is set. Weighted toward DENY paths on purpose — a bug in an authority check fails OPEN and nothing about the request looks wrong. Covers: unreadable artifact reports `not_found` not `permission_denied`; cannot file into a space the caller does not hold; a zero-grant caller can classify nothing; trusted local CLI is exempt from the grant check; REMOVAL is never grant-gated (blocking it would strand an artifact in an audience nobody present can clear); the last-label guard refuses BEFORE writing so a partial strip cannot orphan an artifact; unknown spaces are rejected rather than minted; both ops stay `users_admin`.
- `test/guest-read-path.test.ts` — the guest read path end to end through the REAL MCP dispatcher, not op handlers directly (calling a handler skips the role drop AND the source-list resolution, testing a path no client takes). Every other v129 test proves the DATABASE enforces labels; none prove a guest can actually READ, because they drive raw SQL or hand the caller its source id alongside the label. A real guest holds ONLY a label — the backfill labels every page with its source, so granting the source id would grant the whole corpus. Pins `get_page` returning the shared artifact, `list_pages` enumerating exactly it (the web UI's whole roster comes from that op), and the unlabelled artifact reading as absence rather than refusal.
